<?php
namespace lib;

error_reporting(0);
class Music
{
    private $_NONCE='0CoJUm6Qyw8W8jud';
    private $_secretKey='TA3YiYCfY2dDJQgg';
    private $_encSecKey='';
    private $_VI='0102030405060708';
    private $_REFERER='http://music.163.com/';
    private $_COOKIE='os=pc; osver=Microsoft-Windows-10-Professional-build-10586-64bit; appver=2.0.3.131777; channel=netease; __remember_me=true;';
    private $_USERAGENT='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.61 Safari/537.36';

    public function login($cell,$pwd,$countrycode="86"){
        $url="https://music.163.com/weapi/w/login/cellphone?csrf_token=";
        $data=array(
        "phone"=>$cell,
        "countrycode"=>$countrycode,
        "password"=>md5($pwd),
        "rememberLogin"=>"true");
        return $this->curl($url,$this->prepare($data));
    }

    private function personalized($limit,$cookie){
        $url="https://music.163.com/weapi/personalized/playlist";
        $data=array(
            "limit"=>$limit,
            "total"=>"true",
            "n"=>1000,);
        $json = $this->curl($url,$this->prepare($data),$cookie);
        $json = json_decode($json['body'],true);
		foreach ($json["result"] as $i => $k) {
			$id[$i] = $k["id"];
		}
		return $id;
    }

    private function getsongid($playlist_id,$cookie){
        $url='https://music.163.com/weapi/v3/playlist/detail?csrf_token=';
        $data=array(
            'id'=>$playlist_id,
            'n'=>1000,
            'csrf_token'=>'',
        );
        $raw=$this->curl($url,$this->prepare($data),$cookie);
        $json=json_decode($raw['body'],true);
      	
		return $json["playlist"]["trackIds"];
        //return json_decode($raw,1)[];
    }

    public function sign($cookie){
        $url="https://music.163.com/weapi/point/dailyTask";
        $data=array("type"=>0);
        return $this->curl($url,$this->prepare($data),$cookie);
    }

    public function daka_new($cookie){
        $playlist = $this->personalized(100,$cookie);
        $ids = array();
        $count=0;
        for ($i = 0; sizeof($ids) < 1000; $i++) {
        	$songid = $this->getsongid($playlist[rand(0,sizeof($playlist)-1)],$cookie);
        	for ($k=0;sizeof($ids) < 1000&&$k<sizeof($songid);$k++) {
        	
        	$ids[$count]["action"]="play";
        	$ids[$count]["json"]["download"] =0 ;
        	$ids[$count]["json"]["end"] ="playend"; 
     		$ids[$count]["json"]["id"] = $songid[$k]["id"];
     		$ids[$count]["json"]["sourceId"] ="";
     		$ids[$count]["json"]["time"] = 240;
     		$ids[$count]["json"]["type"] ="song";
     		$ids[$count]["json"]["wifi"] =0;
     		$count++;
    	    }
        }
        $data =json_encode($ids);
        $url = "http://music.163.com/weapi/feedback/weblog";
        $this->curl($url,$this->prepare(array("logs"=>$data)),$cookie);
        return '{"code":200,"count":'.$count.'}';
    }

    public function daka($csrf,$cookie){
        $playlist = $this->recommend($csrf,$cookie);
        $ids = array();
        for ($i = 0; sizeof($ids) < 300; $i++) {
        	$songid = $this->playlist($playlist[rand(0,sizeof($playlist))],$cookie);
        	foreach ($songid as $id) {
     		$ids[] = $id;
    	    }
        }
        return '{"code":200,"count":'.$this->scrobble($ids,$cookie).'}';
    }

    private function scrobble($songid,$cookie){
        $url="http://music.163.com/weapi/feedback/weblog";
        $res = array();
        $count=0;
        $cookies=$cookie;
		$mh = curl_multi_init();
		foreach ($songid as $k => $id) {
   	        $data[$k]=$this->prepare(array("logs"=>'[{"action":"play","json":{"download":0,"end":"playend","id":"'.$id.'","sourceId":"","time":240,"type":"song","wifi":0}}]'));
            $data[$k]=http_build_query($data[$k]);
			$conn[$k] = curl_init($k);
			curl_setopt($conn[$k], CURLOPT_URL, $url);
			curl_setopt($conn[$k],CURLOPT_POST,1);
			curl_setopt($conn[$k],CURLOPT_POSTFIELDS,$data[$k]);
			//curl_setopt($conn[$k], CURLOPT_TIMEOUT, $timeout);
			curl_setopt($conn[$k], CURLOPT_COOKIE, $cookies);
			curl_setopt($conn[$k], CURLOPT_RETURNTRANSFER, 1);
			curl_multi_add_handle($mh, $conn[$k]);
			$count++;
		}
		do {
			$mrc = curl_multi_exec($mh, $active);
		}
		while ($mrc == CURLM_CALL_MULTI_PERFORM);
		while ($active and $mrc == CURLM_OK) {
			if (curl_multi_select($mh) != -1) {
				do {
					$mrc = curl_multi_exec($mh, $active);
				}
				while ($mrc == CURLM_CALL_MULTI_PERFORM);
			}
		}
		foreach ($array as $k => $value) {
			curl_error($conn[$k]);
			$res[$k] = curl_multi_getcontent($conn[$k]);
			//$header[$k] = curl_getinfo($conn[$k]);
			curl_close($conn[$k]);
			curl_multi_remove_handle($mh, $conn[$k]);
		}
		curl_multi_close($mh);
        return $count;
    }

    private function playlist($playlist_id,$cookie){
        $url='https://music.163.com/weapi/v3/playlist/detail?csrf_token=';
        $data=array(
            'id'=>$playlist_id,
            'n'=>1000,
            'csrf_token'=>'',
        );
        $raw=$this->curl($url,$this->prepare($data),$cookie);
        $json = json_decode($raw['body'],true);
      	foreach ($json["playlist"]["trackIds"] as $i => $k) {
			$ids[$i] = $k["id"];
		}
		return $ids;
        //return json_decode($raw,1)[];
    }

    private function recommend($csrf,$cookie){
        $url="https://music.163.com/weapi/v1/discovery/recommend/resource";
        $json = $this->curl($url,$this->prepare(array('csrf_token'=>$csrf)),$cookie);
        $json = json_decode($json['body'],true);
		foreach ($json["recommend"] as $i => $k) {
			$id[$i] = $k["id"];
		}
		return $id;
    }

    public function level($cookie){
        $url='https://music.163.com/weapi/user/level?csrf_token=';
        return $this->curl($url,$this->prepare(null),$cookie);
    }

    public function detail($uid,$cookie){
        $url="https://music.163.com/weapi/v1/user/detail/${uid}";
        return $this->curl($url,$this->prepare(null),$cookie);
    }

    /**
     * 加密
     *
     * @param [type] $raw
     * @return void
     */
    private function prepare($raw){
        $data['params']=$this->aes_encode(json_encode($raw),$this->_NONCE);
        $data['params']=$this->aes_encode($data['params'],$this->_secretKey);
        $data['encSecKey']=$this->_encSecKey;
        return $data;
    }

    private function aes_encode($secretData,$secret){
        return openssl_encrypt($secretData,'aes-128-cbc',$secret,false,$this->_VI);
    }

    private function curl($url,$data=null,$cookie=null){
        $curl=curl_init();
        curl_setopt($curl,CURLOPT_URL,$url);
        if($data){
            if(is_array($data))$data=http_build_query($data);
            curl_setopt($curl,CURLOPT_POSTFIELDS,$data);
            curl_setopt($curl,CURLOPT_POST,1);
        }
        curl_setopt($curl,CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl,CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl,CURLOPT_CONNECTTIMEOUT, 10);
        curl_setopt($curl,CURLOPT_REFERER,$this->_REFERER);
        curl_setopt($curl,CURLOPT_COOKIE,$this->_COOKIE.$cookie);
        curl_setopt($curl,CURLOPT_USERAGENT,$this->_USERAGENT);
        $httpheader[] = "X-FORWARDED-FOR:".$this->Rand_IP();
        $httpheader[] = "CLIENT-IP:".$this->Rand_IP();
        curl_setopt($curl, CURLOPT_HTTPHEADER, $httpheader);
        curl_setopt($curl,CURLOPT_HEADER,1);
        
        $ret = curl_exec($curl);
        $headerSize = curl_getinfo($curl, CURLINFO_HEADER_SIZE);
        $header = substr($ret, 0, $headerSize);
        $body = substr($ret, $headerSize);
        $ret=array();
        $ret['header']=$header;
        $ret['body']=$body;
        curl_close($curl);
        return $ret;
    }
    
    private function Rand_IP(){
        $ip2id= round(rand(600000, 2550000) / 10000); //第一种方法，直接生成
        $ip3id= round(rand(600000, 2550000) / 10000);
        $ip4id= round(rand(600000, 2550000) / 10000);
        //下面是第二种方法，在以下数据中随机抽取
        $arr_1 = array("218","218","66","66","218","218","60","60","202","204","66","66","66","59","61","60","222","221","66","59","60","60","66","218","218","62","63","64","66","66","122","211");
        $randarr= mt_rand(0,count($arr_1)-1);
        $ip1id = $arr_1[$randarr];
        return $ip1id.".".$ip2id.".".$ip3id.".".$ip4id;
    }
}
?>